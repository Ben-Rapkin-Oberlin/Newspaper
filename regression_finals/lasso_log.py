...
# A constant offset for targets (same as in XGBoost script)
LOG_OFFSET = 1
...
def find_best_lasso_params(X_train, y_train):
    ...
    param_grid = {
        'alpha': [.6],  # Try higher alphas for more regularization .48 ->408
        'max_iter': [30000],  # Increase max iterations
...
def recommend_feature_selection(model, feature_names, top_n=10):
    """
    Analyze feature coefficients and provide recommendations for manual feature selection.
    This can help identify features that might be causing instability.
    """
    # Get coefficients
    coef = model.coef_
    ...
    # Find features with extremely large coefficients
    large_coef = feature_importance[feature_importance['Abs_Coefficient'] > 1.0]
    if not large_coef.empty:
        print("\nFeatures with large coefficients that might need inspection:")
        for idx, row in large_coef.iterrows():
            print(f"  {row['Feature']}: {row['Coefficient']:.6f}")
    
    # Find lag features and their importance
...
def evaluate_lasso_log_rotation(X, y, years, val_size, lasso_params, selected_features=None, cap_predictions=True):
    """
    Evaluate Lasso model on a specific rotation of the data with log-transform on the target.
    ...
    Added option to cap predictions to avoid unrealistic values.
    """
    ...
    # Create and train Lasso model in log-space
    # Maybe increase alpha slightly for more regularization if needed
    ...
    # Cap predictions if needed to avoid overflow
    if cap_predictions:
        train_pred_log = np.clip(train_pred_log, None, 20)  # Cap at exp(20)
        val_pred_log = np.clip(val_pred_log, None, 20)  # Cap at exp(20)
...
def save_model(model, scalers=None, file_path="lasso_log_model.pkl", feature_names=None):
...
def load_model(file_path="lasso_log_model.pkl"):
    ...
    scalers = model_data.get('scalers', None)
    
    if feature_names:
        print(f"Model was trained with {len(feature_names)} features: {', '.join(feature_names)}")
    
    return model_data['model'], scalers, feature_names

def add_early_year_predictions(train_df, model, x_scaler, selected_features=None):
...
def backcast_model(df_desc, n_train, model, x_scaler, feature_names, selected_features=None):
    """
    ...
    For log-transformed LASSO, the features are scaled using x_scaler, and because the model
    was trained on log(y+offset), the prediction is converted back via:
        y_pred = exp(predicted_log) - LOG_OFFSET
    
    Added safeguards to prevent overflow during exponential conversion.
    """
    df = df_desc.copy()
    for i in range(n_train, len(df)):
        if pd.isna(df.loc[i, 'estimated_deaths']):
            ...
            # Scale features and select if needed
            if selected_features is not None:
            ...
            try:
                ...
                # Add safeguards against overflow
                if pred_log > 20:  # exp(20) is already very large
                    print(f"Warning: Very large log prediction at index {i}, year {df.loc[i, 'year']}: {pred_log}")
                    pred_log = 20  # Cap the log value
                
                # Safely convert back from log-space
                try:
                    pred_value = np.exp(pred_log) - LOG_OFFSET
                    
                    # Add an additional sanity check for the predicted value
                    if pred_value > 10000:  # Set a reasonable upper limit
                        print(f"Warning: Prediction at index {i}, year {df.loc[i, 'year']} capped at 10000 (was {pred_value})")
                        pred_value = 10000
                    
                    # Update the DataFrame with the predicted value
                    df.loc[i, 'estimated_deaths'] = pred_value
                except:
                    print(f"Error converting prediction at index {i}, year {df.loc[i, 'year']}. Using a default value.")
                    df.loc[i, 'estimated_deaths'] = df.loc[i-1, 'estimated_deaths'] if i > 0 else 1000  # Use previous value or default
            except Exception as e:
                print(f"Error processing index {i}, year {df.loc[i, 'year']}: {str(e)}. Using a default value.")
                df.loc[i, 'estimated_deaths'] = df.loc[i-1, 'estimated_deaths'] if i > 0 else 1000  # Use previous value or default
...
def main():
    ...
    model_path = os.path.join(output_dir, "lasso_log_model.pkl")
    if os.path.exists(model_path) and os.path.getsize(model_path) > 0:
        print(f"Found existing model at {model_path}")
        try:
            model, scalers, feature_names = load_model(model_path)
    ...
    if retrain:
        ...
        # Scale features for parameter search
        x_scaler = StandardScaler()
        X_scaled = x_scaler.fit_transform(X)
        X_scaled = pd.DataFrame(X_scaled, columns=X.columns)
        
        # Apply log transform to target for parameter search
        y_log = np.log(y + LOG_OFFSET)
        
        best_lasso_params, _ = find_best_lasso_params(X_scaled, y_log)
        
        # Create initial model with best params for feature selection
        initial_model = Lasso(**best_lasso_params, random_state=42)
        initial_model.fit(X_scaled, y_log)
        ...
        # Variables to store the last trained model
        last_model = None
        last_scalers = None
        ...
        for i in range(n_rotations):
            ...
            key = f"Rotation {i+1}"
            print(f"\nEvaluating {key}...")
            results[key] = evaluate_lasso_log_rotation(
                X, y, years,
                val_size=val_size,
        ...
        if last_model is not None and last_feature_names is not None:
            coef_plot = plot_lasso_feature_importance(last_model.coef_, last_feature_names, title="Log-Transformed Lasso Feature Coefficients")
            coef_plot.savefig(os.path.join(output_dir, 'lasso_log_feature_coefficients.png'))
            plt.close()
        ...
        # Create predictions for all training data points
        ...
        all_preds = np.exp(pred_log) - LOG_OFFSET
        ...
        if 1900 not in train_predictions or 1901 not in train_predictions:
            early_year_preds = add_early_year_predictions(train_df, last_model, last_scalers['x'], selected_features)
        ...
        if os.path.exists(pred_path):
            try:
                ...
                # Perform backcasting
                print("\nPerforming backcasting with Log-Transformed LASSO model...")
                df_backcast = backcast_model(combined_desc, n_train, last_model, 
                                           last_scalers['x'], feature_names=last_feature_names, 
                                           selected_features=selected_features)
                ...
                output_csv = os.path.join(data_dir, "backcast_predictions_lasso_log_1870_1899.csv")
        ...
        plt.plot(train_pred_years, [train_predictions[year] for year in train_pred_years], 
                'b^--', alpha=0.7, label='Log-LASSO Predictions (1900-1936)')
        ...
        if backcast_predictions:
            ...
            plt.plot(years_backcast, predictions_backcast, 
                    'r*--', alpha=0.7, label='Log-LASSO Backcast (1870-1899)')
        
        # Add vertical line to separate backcast from actual data
        plt.axvline(x=1899.5, color='gray', linestyle=':', alpha=0.7)
        
        plt.title('Log-Transformed LASSO: Training Predictions and Backcast (1870-1936)')
        plt.xlabel('Year')
        ...
        output_plot = os.path.join(output_dir, 'lasso_log_combined_predictions.png')
        ...
        plt.plot(agg_df.index, errors, 'o-', label='Log-LASSO Error', color='blue', alpha=0.7)
        
        plt.title('Log-Transformed Lasso Regression: Absolute Prediction Errors Over Time')
        plt.xlabel('Year')
        plt.ylabel('Absolute Error')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'lasso_log_errors.png'))
        plt.show()
    else:
        ...
        if run_evaluation:
            ...
            y_pred = np.exp(pred_log) - LOG_OFFSET
            
            # Calculate metrics
            metrics = calculate_metrics(y, y_pred)
            
            print("\nLog-Transformed LASSO Metrics on Full Dataset:")
            print(f"MAE: {metrics['mae']:.2f}")
            print(f"RMSE: {metrics['rmse']:.2f}")
            print(f"R²: {metrics['r2']:.3f}")
            print(f"Median AE: {metrics['median_ae']:.2f}")
            ...
            if feature_names:
                coef_plot = plot_lasso_feature_importance(model.coef_, feature_names, title="Log-Transformed Lasso Feature Coefficients")
                coef_plot.savefig(os.path.join(output_dir, 'lasso_log_feature_coefficients.png'))
                plt.show()
            ...
            # Add predictions for early years
            
            # Add predictions for early years (1900-1901) if they're missing
            if 1900 not in train_predictions or 1901 not in train_predictions:
                early_year_preds = add_early_year_predictions(train_df, model, scalers['x'], feature_names)
                train_predictions.update(early_year_preds)
            ...
            plt.title('Log-Transformed LASSO: Predictions vs Actual (1900-1936)')
            plt.xlabel('Year')
            plt.ylabel('Estimated Deaths')
            plt.legend()
            plt.grid(True)
            ...
            output_plot = os.path.join(output_dir, 'lasso_log_predictions_evaluation.png')
            plt.savefig(output_plot)
            plt.show()
            print(f"Evaluation plot saved to {output_plot}")
            
            # Optional: Run backcasting on the existing model
            run_backcast = input("Would you like to generate backcast predictions with this model? (y/n): ").lower() == 'y'
            if run_backcast:
                pred_path = os.path.join(data_dir, "pred_data_1870_1899.csv")
                if os.path.exists(pred_path):
                    try:
                        ...
                        # Ensure the prediction data has an 'estimated_deaths' column
                        if 'estimated_deaths' not in pred_df.columns:
                            pred_df['estimated_deaths'] = np.nan
                        ...
                        # Sort descending for backcasting
                        combined_desc = combined_df.sort_values('year', ascending=False).reset_index(drop=True)
                        n_train = combined_desc[combined_desc['year'] >= 1900].shape[0]
                        
                        # Perform backcasting
                        print("\nPerforming backcasting with Log-Transformed LASSO model...")
                        df_backcast = backcast_model(combined_desc, n_train, model, 
                                                   scalers['x'], feature_names=feature_names, 
                        ...
                        # Create visualization with backcasts
                        backcast_predictions = {year: pred for year, pred in zip(backcast_data['year'], backcast_data['estimated_deaths'])}
                        ...
                        # Save combined plot
                        output_plot = os.path.join(output_dir, 'lasso_log_evaluation_with_backcast.png')
                        plt.savefig(output_plot)
                        plt.show()
                        print(f"Combined evaluation plot with backcast saved to {output_plot}")
